...
self.addEventListener('push', function (e) {
  ...
  /* Раньше признак «чат открыт» читался только из client.url (метка
     ...
     postMessage и ждёт ответа, а метка в адресе осталась только как
     запасной вариант, если окно почему-то не ответило.

     Таймаут ожидания — 900 мс (раньше было 400 мс). 400 мс оказалось
     мало: если в момент push главный поток страницы был занят (идёт
     renderChat()/renderToday() — не редкость на этом же самом событии,
     ведь пуш обычно приходит ОДНОВРЕМЕННО с самим сообщением через
     Centrifugo, а значит и с перерисовкой ленты), ответ на postMessage
     не успевал долететь за 400 мс, SW тихо считал "не ответили" и
     проваливался в ненадёжный fallback по #chat-open — и уведомление
     показывалось человеку, который на самом деле в этот момент СМОТРЕЛ
     в чат. 900 мс — с запасом на любую разумную перерисовку, при этом
     push всё равно не "висит" ощутимо для получателя. */
